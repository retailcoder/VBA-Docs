...
An interface is a collection of prototypes representing the members (methods and properties) the interface is abstracting; that is, it contains only the declarations for the member procedures. A class provides an implementation of all of the methods and properties of one or more interfaces. Classes provide the code used when each function is called by a controller of the class. All classes implement at least one interface, which is considered the default interface of the class. In Visual Basic, any member that isn't explicitly a member of an implemented interface is implicitly a member of the default interface.
 ...
 **Note**  A class or interface that **Implements** an interface does not expose that interface to other classes that implement that class: interface and class inheritance are not supported in VBA.

When you implement an interface or class, you must include all the  **Public** procedures involved. A missing member in an implementation of an interface or class causes a compilation error. If you don't place code in one of the procedures in a class you are implementing, you can raise the appropriate error (run-time error 445, or **Const** E_NOTIMPL = &H80004001) so a user of the implementation understands that a member is not supported.

The  **Implements** statement can't appear in a [standard module](../../Glossary/vbe-glossary.md#standard-module).

Public fields in an interface must be implemented as read/write **Property** members.

## Example

The following example shows how to use the  **Implements** statement to make a set of declarations available to multiple classes. By sharing the declarations through the **Implements** statement, neither class has to make any declarations itself. The example also shows how use of an interface enables *polymorphism*: a strongly-type variable can be declared using the interface type. It can then be assigned objects of different class types that implement the interface.
...
Private sup As Supplier 

Private Sub UserForm_Initialize()
 Set cust = New Customer
 Set sup = New Customer
End Sub

Private Sub Customer_Click() 
 With New DataEntryForm 
  Set .PD = cust 
  .Show 1 
 End With
End Sub 
...
Public Property Set PD(ByVal Data As PersonalData) 
    Set m_pd = Data
    SetTextFields
End Property
